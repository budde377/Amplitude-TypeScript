...
// Soft byte-size cap for merging same-session contexts after a throttle pause.
...
// the server's 10MB-compressed 413 ceiling. Compared against UTF-8 byte measurement
// (via Blob) to match the per-sequence limit's enforcement in base-events-store.ts.
